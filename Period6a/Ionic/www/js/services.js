angular.module('starter.services', [])
  ...
  .factory("Login", function ($firebaseArray) {
    return {
      login: function (user) {
        ...
        var account = new Firebase("https://examprep6.firebaseio.com/accounts/" + user.uid);
        if (account == null) {
          account.set(userData);
        }

        //add user to online userlist
        var loginUser = new Firebase("https://examprep6.firebaseio.com/online/" + user.uid);
        loginUser.set(userData);
      },
      logout: function (user) {
        //Remove user from online userlist
        var removeUser = new Firebase("https://examprep6.firebaseio.com/online/" + user.uid);
        removeUser.remove();
      ...
      chats: function (user) {
        var chats = new Firebase("https://examprep6.firebaseio.com/accounts/" + user.uid + "/chats");
  ...
  .factory("Users", function ($firebaseArray) {
    var onlineUsers = new Firebase("https://examprep6.firebaseio.com/online");
